...
else
{
	...
	$result=mysql_query("select * from reservation where Reservation='$reservation'");
...
<table width="1024" border="0" align="center" cellspacing="0">
  ...
  <tr>
      ...
      <table width="80%" border="5" class="myBorder" bordercolor="#C3DFFF" frame="box">
        <tr bgcolor="#C3DFFF">
          <td align="middle">预约申请详情</td>          
        </tr>        
        <tr bgcolor="#FFFEEC">
          <td>
            <table width="100%" border="1" class="myBorder" bordercolor="#C3DFFF">
              <tr>                
                <td width="50"><?php echo "预约单号：".$reservation;?></td>
                <td width="50"><?php echo "报销项目号：".$project;?></td>
              </tr>
              <tr>
                <td width="50"><?php echo "预约时间：".$date;?></td>
                <td width="50">
				<?php 
					if(!strcmp($state,"reserving"))
						echo "当前状态：预约中";					
					else echo "当前状态：已入账";
				?>
                </td>
          ...
          <?php
		    $number_total=0;
			 ...
			 for($i=0;$i<$count_catalog;$i++)
			 {
				...
				$result=mysql_query("select accountID,userID,count(*),sum(Money*100)/100 from account where reservation='$reservation' and catalog='".($i+1)."' group by accountID order by accountID");
        ...
        <tr>
          ...
          <?php		  	
			if(!strcmp($state,"reserving"))			
				echo "<input type='button' value='确认入账' onclick=\"window.location.href='admin_done_reservation.php?reservation=".$reservation."'\" />";				
