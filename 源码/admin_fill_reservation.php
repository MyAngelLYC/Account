...
else
{
	...
	if(((int)$count)<=0)
	{
		setcookie("add_reservation","empty",time()+600);
		Header("Location: admin_add_reservation.php");
	}
	for($i=0;$i<$count;$i++)
		$account[]=str_split($chosen[$i],15);
	$link=mysql_connect("localhost","root","123") or die("链接出错：".mysql_error());
	mysql_select_db('Account');
	mysql_query("set names utf8");
	mysql_query("delete from reservation_confirm");
	for($i=0;$i<$count;$i++)
		mysql_query("insert into reservation_confirm (accountID,catalog) values('".$account[$i][0]."','".$account[$i][1]."')");
	mysql_close($link);	
}
?>
...
<table width="1024" border="0" align="center" cellspacing="0">
  ...
  <tr>
      ...
      <table width="80%" border="5" class="myBorder" bordercolor="#C3DFFF" frame="box">
        <tr bgcolor="#C3DFFF">
          <td align="middle" colspan="5"><font size="+1">请核对预约项目并填写预约信息</font></td>          
        </tr> 
        <tr bgcolor="#FFFEEC">          
         ...
         <form action="admin_confirm_reservation.php" method="post">
          <?php
			 $number_total=0;
			 $money_total=0;
			 $link=mysql_connect("localhost","root","123") or die("链接出错：".mysql_error());
        ...
        <tr>
          <td align="center" colspan="5">                    
          	<table border="0" width="100%">
            	<tr>
                	<td align="right" width="50%">预约单号：<input type="text" name="reservation" /></td>
                    <td align="left" width="50%">报销项目号：<input type="text" name="project" /></td>
                 </tr>
            </table>         
          </td>
        </tr>          
        <tr>
          <td align="center" colspan="5">                    
          <input type="button" value="返回" onclick="window.location.href='admin_add_reservation.php'" />
